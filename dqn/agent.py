...
class Agent(BaseModel):
    ...
    def train(self):
        ...
        for self.step in tqdm(range(start_step, self.max_step), ncols=70, initial=start_step):
            ...
            # 2. act
            screen, reward, terminal, info = self.env.act(action, is_training=True)
            # 3. observe
            ...
            if self.step >= self.learn_start:
                if self.step % self.test_step == self.test_step - 1:
                    ...
                    tqdm.write(
                        "\navg_r: {:.4f} avg_l: {:.6f}, avg_q: {:3.6f} avg_ep_r: {:.4f}, max_ep_r: {:.4f}, min_ep_r: {:.4f}, # game: {:,}".format(
                            avg_reward,
                            avg_loss,
                            avg_q,
                            avg_ep_reward,
    ...
    def build_dqn(self):
        ...
        with tf.variable_scope('prediction'):
            ...
            self.l1, self.w['l1_w'], self.w['l1_b'] = conv2d(self.s_t, 32, [8, 8], [4, 4], initializer, activation_fn, self.cnn_format, name='l1')
            self.l2, self.w['l2_w'], self.w['l2_b'] = conv2d(self.l1, 64, [4, 4], [2, 2], initializer, activation_fn, self.cnn_format, name='l2')
            self.l3, self.w['l3_w'], self.w['l3_b'] = conv2d(self.l2, 64, [3, 3], [1, 1], initializer, activation_fn, self.cnn_format, name='l3')
            ...
            if self.dueling:
                ...
                # Average Dueling
                self.q = self.value + (self.advantage - tf.reduce_mean(self.advantage, reduction_indices=1, keepdims=True))
            else:
        ...
        with tf.variable_scope('target'):
            ...
            self.target_l1, self.t_w['l1_w'], self.t_w['l1_b'] = conv2d(self.target_s_t, 32, [8, 8], [4, 4], initializer, activation_fn, self.cnn_format, name='target_l1')
            self.target_l2, self.t_w['l2_w'], self.t_w['l2_b'] = conv2d(self.target_l1, 64, [4, 4], [2, 2], initializer, activation_fn, self.cnn_format, name='target_l2')
            self.target_l3, self.t_w['l3_w'], self.t_w['l3_b'] = conv2d(self.target_l2, 64, [3, 3], [1, 1], initializer, activation_fn, self.cnn_format, name='target_l3')
            ...
            if self.dueling:
                ...
                self.target_q = self.t_value + (
                            self.t_advantage - tf.reduce_mean(self.t_advantage, reduction_indices=1, keep_dims=True))
            else:
    ...
    def play(self, n_step=10000, n_episode=100, test_ep=None, render=False):
        ...
        for idx in tqdm(range(n_episode), desc="Episode"):
            screen, reward, action, terminal = self.env.new_random_game()
            current_reward = 0
            ...
            with tqdm(total=n_step, desc="Step", leave=False, ncols=70) as tqdm_step:
                for t in range(n_step):
                    tqdm_step.update()

                    action = self.predict(test_history.get(), test_ep)
                    # 2. act
                    screen, reward, terminal, info = self.env.act(action, is_training=False)
                    # 3. observe
                    test_history.add(screen)
            ...
            output_history.append((current_reward, t, info))

            if current_reward > best_reward:
                best_reward = current_reward
                best_idx = idx

            tqdm.write("[+] {:,} Episode END\n\tReward: {:,}\n\tNum of Step: {:,}\n\tinfo: {}\n".format(idx, current_reward, t, str(info)))
            tqdm.write("\t%dth Best reward : %d" % (best_idx, best_reward))
            tqdm.write("=" * 30)
            tqdm.write("\n")

            with open("{}.txt".format(self.env_name), "at") as f:
                f.write(json.dumps((current_reward, t, info)) + "\n")
